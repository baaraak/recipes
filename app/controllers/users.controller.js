...
class UsersController extends BaseController {
  ...
  signup = async (req, res, next) => {
    ...
    if (!validator.firstName(firstName))
      return next(createError('not valid firstName'));
    if (!validator.lastName(lastName))
      return next(createError('not valid lastName'));
    if (!validator.email(email)) return next(createError('not valid email'));
    if (!validator.country(country))
      return next(createError('not valid country'));
    if (!validator.password(password))
      return next(createError('not valid password'));
  ...
  update = async (req, res, next) => {
    ...
    User.findOne({ email }, (err, user) => {
      if (user.length > 0 && user._id !== req.currentUser._id)
        return next(createError('Email already exist'));
    });
    User.findOneAndUpdate(
